...
<body class="min-h-screen flex flex-col selection:bg-emerald-500/20 selection:text-emerald-300">
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2.5 font-mono text-xs">
                ...
                >
                    ...
                    <span class="font-sans font-medium text-xs hidden md:inline max-w-[100px] truncate">{{ $user->name }}</span>
                </button>
    ...
    <main class="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <!-- 2FA Encouragement Banner (If operative hasn't configured 2FA yet) -->
        @if(! $canUsePinlessEntry)
            <div class="rounded-2xl bg-gradient-to-r from-amber-950/40 via-slate-900/60 to-amber-950/20 border border-amber-500/30 p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 backdrop-blur-md">
                <div class="flex items-center gap-3.5">
                ...
                <button
                    type="button"
                    onclick="openProfileModal()"
                    class="px-4 py-2 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-200 text-xs font-medium cursor-pointer transition-colors shrink-0 flex items-center gap-1.5"
                >
                    ...
                    <span>{{ __('Enable 2FA / Biometrics') }}</span>
                </button>
            </div>
        @endif
        ...
        <div class="space-y-4">
                                ...
                                <form method="POST" action="{{ route('channels.enter', $channel->id) }}">
                                    ...
                                    @if($canUsePinlessEntry)
                                        ...
                                        >
                                            ...
                                            <span>{{ __('Enter Stream (Pinless)') }}</span>
                                        </button>
                                        <div class="text-[10px] text-emerald-400/80 text-center font-mono mt-1.5 flex items-center justify-center gap-1">
                                            ...
                                            <span>{{ __('2FA Verified • Instant Access') }}</span>
                                        </div>
                                    @else
                                        <button
                                            type="submit"
                                        ...
                                        >
                                            ...
                                            <span>{{ __('Enter Channel (Requires PIN)') }}</span>
                                        </button>
                                        <div class="text-[10px] text-slate-500 text-center font-mono mt-1.5">
                                            {{ __('Activate 2FA in Profile to skip PIN') }}
                                        </div>
                                    @endif
                                </form>
                            </div>
            ...
            @else
                <!-- Zero Channels Empty State -->
                <div class="rounded-2xl bg-slate-900/40 border border-dashed border-slate-800 p-12 text-center flex flex-col items-center justify-center space-y-4">
    ...
    <div id="redeem-modal" class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm hidden p-4 flex items-center justify-center">
        <div class="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden font-sans">
            ...
            <form method="POST" action="{{ route('channels.redeem') }}" class="p-5 space-y-4">
                @csrf
                <div>
                    <label for="redeem-code-input" class="block text-xs font-medium text-slate-300 mb-1.5 font-mono">
        ...
        <div class="w-full max-w-md my-auto bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden font-sans">
            ...
            <form id="profile-form" onsubmit="saveProfile(event)" class="flex flex-col flex-1 min-h-0">
                ...
                <div class="overflow-y-auto p-4 sm:p-5 space-y-3.5 text-xs font-sans flex-1 modal-scroll">
                    ...
                    <div>
                        <label class="block font-medium text-slate-300 mb-1">{{ __('Primary Email Address') }} <span class="text-rose-400">*</span></label>
                        <input
                            type="email"
                            name="email"
                    ...
                    <div class="pt-3 border-t border-slate-800 space-y-3">
                        <div class="text-[11px] font-semibold text-emerald-400 uppercase tracking-wider font-mono flex items-center justify-between">
                            <span>{{ __('Security Credentials & 2FA') }}</span>
                            @if($canUsePinlessEntry)
                                <span class="text-[10px] text-emerald-400 lowercase font-normal bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-500/30">pinless active</span>
                            @endif
                        </div>
                        ...
                        <div class="p-3 bg-slate-950/60 rounded-xl border border-slate-800 space-y-2">
                            <div class="flex items-center justify-between">
                                <div>
                                    <div class="font-medium text-white flex items-center gap-1.5">
                                        <span>{{ __('Hardware Biometrics / Passkeys') }}</span>
                                    </div>
                                    <div class="text-[10px] text-slate-400 mt-0.5">{{ __('Apple Touch ID, Face ID, Windows Hello') }}</div>
                                </div>
                                <button
                                    type="button"
                            ...
                            @if($user->passkeys && $user->passkeys->isNotEmpty())
                        ...
                        <div class="p-3 bg-slate-950/60 rounded-xl border border-slate-800 space-y-2">
                            <div class="flex items-center justify-between">
                                <div>
                                    <div class="font-medium text-white">{{ __('Authenticator App (TOTP)') }}</div>
                                    <div class="text-[10px] text-slate-400 mt-0.5">{{ __('Google Authenticator, 1Password, Authy') }}</div>
                                </div>
                            ...
                            <!-- Interactive 2FA Setup Container -->
                            <div id="totp-setup-container" class="hidden pt-3 border-t border-slate-800 space-y-3">
                                ...
                                <div class="flex gap-2">
                                    <input
                                        ...
                                        maxlength="6"
                                        placeholder="123456"
                                        class="w-full px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-center text-sm font-mono tracking-widest text-slate-100"
                                    >
                                    <button
                                        type="button"
                                        onclick="confirmTwoFactorSetup()"
                                        class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-medium cursor-pointer"
                                    >
                                        {{ __('Confirm') }}
                                    </button>
                                </div>
                        ...
                        <div class="p-3 bg-slate-950/60 rounded-xl border border-slate-800 space-y-2">
                            <div>
                                <label class="block font-medium text-white mb-1">{{ __('Emergency Recovery Email') }}</label>
                                <div class="text-[10px] text-slate-400 mb-2">{{ __('Alternative contact email to recover access if 2FA credentials are lost.') }}</div>
                                <div class="flex gap-2">
                                    <input
                                        type="email"
                                        ...
                                        class="w-full px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-slate-100 text-xs focus:outline-none focus:border-emerald-500/60"
                                    >
                                    <button
                                        type="button"
                    ...
                    <div class="pt-3 border-t border-slate-800 space-y-2.5">
                        <div class="text-[11px] font-semibold text-slate-400 font-mono">{{ __('Change Password') }}</div>
                        <input
                            type="password"
                            name="current_password"
                            id="profile-input-current-pass"
                            placeholder="{{ __('Current Password') }}"
                            class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-slate-100 placeholder-slate-600 focus:outline-none focus:border-emerald-500/60"
                        >
                        <input
                            type="password"
                            name="password"
                ...
                <div class="p-4 sm:p-5 pt-3 sm:pt-4 border-t border-slate-800 flex items-center justify-end gap-2 shrink-0 bg-slate-900">
                    <button
                        type="button"
    ...
    <script>
        ...
        function markAvatarForRemoval() {
            ...
            document.getElementById('profile-avatar-preview-img').src = '';
            document.getElementById('profile-avatar-preview-img').classList.add('hidden');
        ...
        async function disableTwoFactor() {
            if (!confirm('Are you sure you want to disable 2FA? Pinless entry to channels will be revoked.')) return;
            try {
        ...
        async function saveRecoveryEmail() {
            ...
            try {
                ...
                const data = await res.json();
                window.alert(data.message || (res.ok ? 'Recovery email saved!' : 'Error'));
            } catch(e) {
                window.alert('Failed to save recovery email');
            }
        }
        ...
        async function registerPasskey() {
            try {
                ...
                if (window.SimpleWebAuthnBrowser) {
                    ...
                    const verRes = await fetch('{{ route("webauthn.register") }}', {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Content-Type': 'application/json',
                    ...
                    if (verRes.ok) {
                        ...
                        window.alert(verData.message || 'Passkey registration failed');
                    }
                } else {
                    window.alert('WebAuthn is not supported on this browser or device.');
                }
            } catch(e) {
                console.error(e);
            }
        ...
        async function deletePasskey(id) {
            ...
            try {
                ...
                window.alert('Failed to delete passkey');
            }
        }
    </script>
</body>
